...
public class HashedArrayIndexedUniverse<T> extends IndexFunctionIndexedUniverse<T> {
	private HashedArrayIndexedUniverse(Object[] hashedElementArray, HashIndexedFunction function, int elementCount) {
		super(hashedElementArray, function, elementCount);
	}
	
	/**
	 * Creates a hash-based {@link IndexedUniverse} that contains all elements from {@code values} 
	 ...
	 * 
	 * @return a Hash-based {@link IndexedUniverse} or <code>null</code> if that is not possible. 
	 */
		...
		do {
			...
			for (int i = 0; i < hashFunctions.size(); i++) {
				...
				if (mappedUniquelyInTarget(values, target, hashFunction)) {
					return new HashedArrayIndexedUniverse<T>(target, hashFunction.indexFunction(size), elementCount);
				}
				nullArray(target);
			}
			size <<= 1;
	...
	public static <T> IndexedUniverse<T> createFrom(Set<T> values, int powerShifts) {
		return createFrom(values, powerShifts, HashFunction.DEFAULT_SAFE_HASHES);
	}
	
	public static <T> IndexedUniverse<T> createFrom(Set<T> values) {
	...
	private static int equalOrGreaterPowerOfTwo(int elementCount) {
		if (elementCount > HashIndexedFunction.MAX_SIZE) {
			throw new IllegalArgumentException("Size (" + elementCount + ") exceeds the maximum number of elements in a " + HashedArrayIndexedUniverse.class.getSimpleName() + " (" + HashIndexedFunction.MAX_SIZE + ")");
		}
		int size = 1;
